...
export function useChannels() {
  ...
  const load = useCallback(async () => {
    try {
      const { channels } = await authFetch(`${API}/channels`);
   ...
   */
  const createChannel = useCallback(async (name, description = "", is_private = 0) => {
    const { channel } = await authFetch(`${API}/channels`, {
      method: "POST",
  ...
  const updateChannel = useCallback(async (id, updates) => {
    const { channel } = await authFetch(`${API}/channels/${id}`, {
      method: "PATCH",
...
export function useMessages(channelId) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    authFetch(`${API}/messages/${channelId}`)
  ...
  const loadMore = useCallback(async () => {
    ...
    setMessages((prev) => {
      ...
      const oldest = prev[0];
      authFetch(`${API}/messages/${channelId}?before=${oldest.id}`)
   ...
   */
  const updateMessage = useCallback((partial) => {
    setMessages((prev) =>
 ...
 * }}
 */
export function useDm(partnerId) {
  const { authFetch } = useAuth();
  const [messages, setMessages] = useState([]);
  const [loading, setLoading]   = useState(false);
  const [hasMore, setHasMore]   = useState(false);
  ...
  const loadMore = useCallback(async () => {
    ...
    setMessages((prev) => {
      if (!prev.length) return prev;
      const oldest = prev[0];
      authFetch(`${API}/dm/${partnerId}?before=${oldest.id}`)
        .then(({ messages: older, hasMore: more }) => {
          setMessages((cur) => [...older, ...cur]);
          setHasMore(more);
        })
...
export function useConversations() {
  ...
  const load = useCallback(async () => {
    try {
      const { conversations } = await authFetch(`${API}/dm/conversations`);
...
export function usePinnedMessages(channelId) {
  ...
  useEffect(() => {
    ...
    authFetch(`${API}/messages/${channelId}/pinned`)
   ...
   */
  const addPinned = useCallback((msg) => {
   ...
   * @param {number} messageId
   */
  const removePin = useCallback((messageId) => {
...
export function useUserSearch() {
  ...
  const search = useCallback(async (q) => {
    ...
    setLoading(true);
    try {
      const data = await authFetch(`${API}/auth/search?q=${encodeURIComponent(q)}`);
...
export function useGlobalSearch() {
  ...
  const search = useCallback(async (q) => {
    ...
    setLoading(true);
    try {
      const data = await authFetch(`${API}/messages/search?q=${encodeURIComponent(q)}`);
...
export function useChannelMembers(channelId) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { members } = await authFetch(`${API}/channels/${channelId}/members`);
      setMembers(members);
   ...
   */
  const addMember = useCallback(async (username) => {
    const data = await authFetch(`${API}/channels/${channelId}/members`, {
      method: "POST",
 ...
 * }}
 */
export function useChannelPermissions(channelId) {
  const { authFetch } = useAuth();
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { permissions } = await authFetch(`${API}/channels/${channelId}/permissions`);
   ...
   * @returns {Promise<void>}
   */
  const updateRole = useCallback(async (role, perms) => {
 ...
 * }}
 */
export function useChannelInvites(channelId) {
  const { authFetch } = useAuth();
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { invites } = await authFetch(`${API}/channels/${channelId}/invites`);
      setInvites(invites);
    } catch { setInvites([]); }
  }, [channelId, authFetch]);
  ...
  const createInvite = useCallback(async (opts = {}) => {
    const { invite } = await authFetch(`${API}/channels/${channelId}/invites`, {
      method: "POST",
   ...
   * @returns {Promise<void>}
   */
  const deleteInvite = useCallback(async (code) => {
 ...
 * }}
 */
export function useSearch(channelId) {
  const { authFetch } = useAuth();
  const [results, setResults] = useState([]);
  const [loading, setLoading] = useState(false);
  const [query, setQuery]     = useState("");
  ...
  const search = useCallback(async (q) => {
    ...
    setLoading(true);
    try {
      const data = await authFetch(`${API}/messages/${channelId}/search?q=${encodeURIComponent(q)}`);
      setResults(data.results);
    } catch (e) {

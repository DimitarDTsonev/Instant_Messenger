 ...
 */
type MessageDivider = { type: "divider"; label: string; key: string };
type MessageGroup = {
  type: "group";
  userId?: number;
  username?: string;
...
type GroupedMessageItem = MessageDivider | MessageGroup;

function groupMessages(messages: Message[]): GroupedMessageItem[] {
  ...
  for (const msg of messages) {
    ...
    if (lastGroup && lastGroup.userId === authorId) {
      ...
      lastGroup = { type: "group", userId: authorId, username: msg.username, avatar: msg.avatar, role: msg.role, messages: [msg], key: `g-${msg.id}` };
...
function MessageRow({
  ...
  function saveEdit() {
    ...
    doEdit(msg.id, trimmed, ({ error }: SocketAck = {}) => {
  ...
  function handleDelete() {
    ...
    doDelete(msg.id, ({ error }: SocketAck = {}) => {
  ...
  function handleReact(emoji: string) {
    doReact(msg.id, emoji, ({ error }: SocketAck = {}) => {

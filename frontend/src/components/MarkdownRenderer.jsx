 ...
 */
function parseInline(text, users, key = 0) {
  if (!text) return null;
  ...
  const patterns = [
    ...
    { re: /@(\w+)/, wrap: (m, k) => {
      const isKnown = users?.some((u) => u.username.toLowerCase() === m[1].toLowerCase());
      return isKnown
        ? <span key={k} style={mentionStyle}>@{m[1]}</span>
        : <span key={k} style={{ color: "#949ba4" }}>@{m[1]}</span>;
    }},
  ];
 ...
 */
export default function MarkdownRenderer({ text, users = [] }) {
  if (!text) return null;

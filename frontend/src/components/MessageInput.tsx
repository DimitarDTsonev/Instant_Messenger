 ...
 */
function fileIcon(filename: string | null | undefined = "") {
 ...
 */
function useDebounce<TArgs extends unknown[]>(fn: (...args: TArgs) => void, delay: number) {
...
export default function MessageInput({
  channelId, channelName, dmUser, isDm = false, disabled = false, canWrite = true, onAddMessage,
  ...
  useEffect(() => {
    ...
    function onDoc(e: globalThis.MouseEvent) {
      if (panelRef.current && e.target instanceof Node && !panelRef.current.contains(e.target)) setShowEmoji(false);
    }
        ...
        <div style={s.mentionDropdown}>
          {mentionList.map((u, i) => (
            <div
              ...
              onMouseDown={(e: MouseEvent<HTMLDivElement>) => { e.preventDefault(); insertMention(u.username); }}
            >
          ...
          <div style={s.emojiGrid}>
            {EMOJI_CATS[emojiCat].map((emoji) => (
              <button
                key={emoji}
                style={s.emojiItem}
                onMouseDown={(e: MouseEvent<HTMLButtonElement>) => { e.preventDefault(); insertEmoji(emoji); }}
                title={emoji}
              >
                {emoji}
              </button>

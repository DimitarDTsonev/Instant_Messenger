...
describe("GET /api/dm/:userId", () => {
  ...
  test("attaches reactions map to messages", async () => {
    const msgId = seedDm(alice.id, bob.id, "hi");
    db.prepare("INSERT INTO dm_reactions (message_id, user_id, emoji) VALUES (?, ?, ?)")
      .run(msgId, alice.id, "+1");
    db.prepare("INSERT INTO dm_reactions (message_id, user_id, emoji) VALUES (?, ?, ?)")
      .run(msgId, bob.id, "+1");
    ...
    expect(res.body.messages[0].reactions["+1"]).toEqual([alice.id, bob.id]);
  });

  test("falls back to default limit when limit is invalid", async () => {
    seedDm(alice.id, bob.id, "hi");
    const res = await request(app).get(`/api/dm/${bob.id}?limit=abc`)
      .set("Authorization", `Bearer ${aliceToken}`);
    expect(res.status).toBe(200);

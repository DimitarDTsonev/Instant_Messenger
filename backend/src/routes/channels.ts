// ============================================================
//  src/routes/channels.ts — Channel management routes
//
...
//
//  Exports (for socket/handlers.ts):
...
export function getUserRole(db: Db, userId: number | string, channelId: number | string): ChannelRole | null {
  ...
  const ch = db.prepare("SELECT created_by, is_private FROM channels WHERE id = ?").get(channelId) as { created_by: number; is_private: number } | undefined;
  if (!ch) return null;
 ...
 */
const DEFAULT_PERMS: Record<Exclude<ChannelRole, "owner">, PermissionSet> = {
...
router.get("/:id/permissions", (req, res) => {
  ...
  const rows = db.prepare("SELECT * FROM channel_permissions WHERE channel_id = ?").all(id) as Array<PermissionSet & { role: Exclude<ChannelRole, "owner">; channel_id: number }>;
...
// -------------------------------------------------------
//  Export router and helpers (helpers used by socket/handlers.ts)
// -------------------------------------------------------

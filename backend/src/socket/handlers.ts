// ============================================================
//  src/socket/handlers.ts — Socket.io event handlers
//
...
import { isSocketRateLimited, isUserBanned, logSecurityEvent, RATE_BAN } from "../middleware/security";
...
type MessageReactionPayload = MessageIdPayload & {
  emoji: string;
};
...
// JWT secret must match the one used in middleware/auth.ts
...
function buildOnlinePayload(): Array<{ id: number; status: OnlineUser["status"] }> {
 ...
 * @returns {void}
 */
function ack(fn: unknown, data: unknown) {
 ...
 */
function getReactions(db: Db, messageId: number): ReactionMap {
  const rows = db
 ...
 * @returns {{ [emoji: string]: number[] }} Map of emoji to arrays of user IDs
 */
function getDmReactions(db: Db, messageId: number): ReactionMap {
  const rows = db
 ...
 */
function getFullMessage(db: Db, messageId: number | bigint): MessageRow | undefined {
 ...
 */
function getFullDmMessage(db: Db, messageId: number | bigint): MessageRow | undefined {
  const msg = db.prepare(`
    SELECT
 ...
 * @returns {void}
 */
function emitToDmPair(io: Server, senderId: number, receiverId: number, event: string, data: unknown) {
 ...
 * @returns {void}
 */
function notifyMentions(io: Server, db: Db, content: string, message: MessageRow, senderUser: Pick<AuthUser, "id" | "username">) {
  const mentionRegex = /@(\w+)/g;
  let match: RegExpExecArray | null;
  while ((match = mentionRegex.exec(content)) !== null) {
    const username = (match[1] || "").toLowerCase();
    const mentioned = db.prepare("SELECT id FROM users WHERE LOWER(username) = ?").get(username) as { id: number } | undefined;
...
export function registerSocketHandlers(io: Server) {
  ...
  io.on("connection", (socket: AuthedSocket) => {
    ...
    socket.on("dm:react", ({ messageId, emoji }: MessageReactionPayload, callback?: AckFn) => {
      const db  = getDb();
      const msg = db.prepare("SELECT sender_id, receiver_id FROM direct_messages WHERE id = ?").get(messageId) as { sender_id: number; receiver_id: number } | undefined;
      if (!msg) return ack(callback, { error: "Message not found" });
